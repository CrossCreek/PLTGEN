...
SSARSensor::SSARSensor(const SSARSensor &copyMe)
...
bool
SSARSensor::operator == (const SSARSensor &compareMe) const
{
...
void
SSARSensor::ExtractSSARSensorInfo(shared_ptr<InputDataElement> ptrInputDataElement)
{
    int        typeIndex = 0;
    int        index = 0;
...
void
SSARSensor::CheckInputData(bool runTimelineGenerator) const
{
    string errorString = "";
...
void
SSARSensor::PrintAttributes(ostream &outfile) const
{
    int        typeIndex = 0;
    const int  numberOfQualities = Target::GetNumberOfQualities();
...
double
SSARSensor::GetCollectionRate(int missionIndex,
...
double
SSARSensor::GetResourceWeight(int missionIndex,
                              int qualityIndex) const
{
...
int
SSARSensor::GetCollectionRateTypeIndex(const string &typeString)
{
    int typeIndex = -1;
...
int
SSARSensor::ConvertCollectionRateIndex(int missionIndex)
{
...
string
SSARSensor::GetCollectionRateString(int typeIndex)
{
...
/**
* Method to return the index which maps to the target deck and the SENSOR enum.  In the case of the SSAR Sensor
* the sensor type of SSAR is always returned.
SSAR = 11,
*
* @return int sensor Index
*/
int
SSARSensor::GetSensorIndex() const
{
    return(SSAR);
}
